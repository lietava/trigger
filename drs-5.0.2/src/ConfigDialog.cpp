 ...
 * $Id: ConfigDialog.cpp 21309 2014-04-11 14:51:29Z ritt $
 */
...
ConfigDialog_fb( parent )
{
   ...
   m_cbMulti->SetValue(m_frame->GetMultiBoard());
...
{
   if (event.GetId() == ID_CH_RANGE)
      m_firstChannel = m_rbChRange->GetSelection();
   else if (event.GetId() == ID_CH_HALF)
