...
//==============================================================================================
//void printdatetime(char* dmyhms)
void printdatetime()
{
 char dmyhms[30];
 time_t T;
 ...
 sprintf(dmyhms,"%2.2d.%2.2d.%4.0d %2.2d:%2.2d:%2.2d",
  ...
  for(int i=0; i<28; i++) { if(dmyhms[i] == ' ') dmyhms[i] = ' '; }; 
  printf("Time: %s \n",dmyhms);
}
...
class AnalyseWaves {
 ...
 public:
	...
	int Compare4Channels();
	int DimSigma(ScopeDimData* dd);
	int DimSigmaEdge(ScopeDimData* dd,ScopeDimData* edge);
	int DimWave(int iwave, ScopeDimWave* w );
        int SaveWaves();
};
    ...
    if((w[i-1]<=thres) && (w[i]>thres)){
      ...
      */
      if(DEBUG){
        ...
        printf("%i %i time %f w0 %f w1 %f  edge %f  \n",iw,nedges,t[i],w[i-1],w[i],edge[nedges]);
      }
      nedges++;
    }
 }
...
int AnalyseWaves::Compare4Channels()
{
 int ret = 0;
...
int AnalyseWaves::DimSigma(ScopeDimData* sig)
{
 Compare4Channels();
 time_t now;
 now=time(0);
...
int AnalyseWaves::DimSigmaEdge(ScopeDimData* sig,ScopeDimData* edge)
{
 Compare4Channels();
 time_t now;
 now=time(0);
...
int AnalyseWaves::DimWave(int iwave, ScopeDimWave* w )
{
 time_t now;
 now=time(0);
 w->time=now;
