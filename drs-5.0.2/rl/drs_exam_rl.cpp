...
int main()
{
   ...
   strcpy(format1,"D:4");
   char format2[8];
   strcpy(format2,"F:2049");
   ScopeDimData* dimsigma = new ScopeDimData;
   ScopeDimData* dimedge = new ScopeDimData;
   ScopeDimWave* dimw1 = new ScopeDimWave; 
   ScopeDimWave* dimw2 = new ScopeDimWave; 
   ScopeDimWave* dimw3 = new ScopeDimWave; 
   ScopeDimWave* dimw4 = new ScopeDimWave; 
   int dimsize1=  sizeof(*dimsigma);
   int dimsize2=  sizeof(*dimw1);
   printf("size of ScopeDimData data: %i \n",dimsize1);
   printf("size of ScopeDimWave data: %i \n",dimsize2);
   DimService scopesigma("ScopeServer/SIGMAS",format1,(void*)dimsigma,dimsize1);
   DimService scopeedge("ScopeServer/EDGES",format1,(void*)dimedge,dimsize1);
   DimService scopewave1("ScopeServer/WAVE1",format2,(void*)dimw1,dimsize2);
   DimService scopewave2("ScopeServer/WAVE2",format2,(void*)dimw2,dimsize2);
   DimService scopewave3("ScopeServer/WAVE3",format2,(void*)dimw3,dimsize2);
   //DimService scopewave4("ScopeServer/WAVE4",format2,(void*)dimw4,dimsize2);
   ...
   if (b->GetBoardType() >= 8) {        // Evaluaiton Board V4&5
      ...
      b->SetTriggerSource(1<<TrigChannel);        // set CH1 as source
   } else if (b->GetBoardType() == 7) { // Evaluation Board V3
      b->EnableTrigger(0, 1);           // lemo off, analog trigger on
      b->SetTriggerSource(0);           // use CH1 as source
   }
   b->SetTriggerLevel(0.0);            // 0.05 V
   ...
   while((signaled==0)) {
	...
	iw++;
	 ...
	 //return 1;
      ...
      if(anal.DimSigmaEdge(dimsigma,dimedge)==0){
 	// waves
        anal.DimWave(0,dimw1);
        scopewave1.updateService();
        anal.DimWave(1,dimw2);
        scopewave2.updateService();
        anal.DimWave(2,dimw3);
        scopewave3.updateService();
        //anal.DimWave(3,dimw4);
        //scopewave4.updateService();
	// analysis
        scopesigma.updateService();
        scopeedge.updateService();
        usleep(500000);
      }
